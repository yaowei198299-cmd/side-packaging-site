...
const InquiryForm = ({ productTitle, lang = 'en' }: { productTitle?: string; lang?: string }) => {
  ...
  const currentCopy = "Fill the form below and our engineering team will provide a cost-optimized blueprint within 24 hours.";
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          <div className="grid md:grid-cols-2 gap-x-10 gap-y-8">
            ...
            <div className="space-y-3">
              ...
              <select required className="w-full px-7 py-4.5 bg-[#111111] border border-white/20 text-white rounded-xl focus:border-[#C9A84C] outline-none transition-all font-semibold text-base appearance-none" value={formData.productType} onChange={(e) => setFormData({...formData, productType: e.target.value})}>
            ...
            <div className="space-y-3">
              ...
              <select required className="w-full px-7 py-4.5 bg-[#111111] border border-white/20 text-white rounded-xl focus:border-[#C9A84C] outline-none transition-all font-semibold text-base appearance-none" value={formData.quantity} onChange={(e) => setFormData({...formData, quantity: e.target.value})}>
            ...
            <div className="space-y-3">
              ...
              <select className="w-full px-7 py-4.5 bg-[#111111] border border-white/20 text-white rounded-xl focus:border-[#C9A84C] outline-none transition-all font-semibold text-base appearance-none" value={formData.materialFinish} onChange={(e) => setFormData({...formData, materialFinish: e.target.value})}>
            ...
            <div className="space-y-3">
              ...
              <select className="w-full px-7 py-4.5 bg-[#111111] border border-white/20 text-white rounded-xl focus:border-[#C9A84C] outline-none transition-all font-semibold text-base appearance-none" value={formData.logoPlacement} onChange={(e) => setFormData({...formData, logoPlacement: e.target.value})}>
            ...
            <div className="space-y-3 md:col-span-2">
              ...
              <input type="text" placeholder="e.g. 4x3x1.25 in (10.2x7.6x3.2 cm)" className="w-full px-7 py-4.5 bg-[#111111] border border-white/20 text-white rounded-xl focus:border-[#C9A84C] outline-none transition-all font-semibold text-base placeholder:text-white/10" value={formData.dimensions} onChange={(e) => setFormData({...formData, dimensions: e.target.value})} />
            </div>
          </div>
          <div className="space-y-3">
            ...
            <textarea rows={4} placeholder="Box size, color, special finish, intended use..." className="w-full px-7 py-5 bg-[#111111] border border-white/20 text-white rounded-2xl focus:border-[#C9A84C] outline-none transition-all font-semibold text-base placeholder:text-white/10 resize-none" value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} />
          </div>
          <div className="space-y-3">
            ...
            <div className="w-full px-7 py-10 bg-white/[0.02] border border-dashed border-white/20 text-center rounded-2xl group hover:border-[#C9A84C]/50 transition-all cursor-pointer">
              <p className="text-white/50 font-bold uppercase text-[10px] tracking-widest group-hover:text-white transition-colors">Click to upload PNG, AI, PDF</p>
            </div>
          </div>
          <div className="pt-6 space-y-10">
            <button type="submit" disabled={isSubmitting} className={`w-full bg-[#C9A84C] text-black font-black uppercase italic tracking-widest py-8 rounded-full hover:bg-white hover:scale-[1.01] active:scale-95 transition-all shadow-2xl flex items-center justify-center space-x-3 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}>
              {isSubmitting ? <span className="animate-pulse text-sm">Processing...</span> : <span className="text-base md:text-lg font-black">SEND MY REQUEST &#8212; FREE & NO COMMITMENT</span>}
            </button>
            <div className="text-center space-y-8 pb-4">
              <p className="text-white/50 text-[11px] md:text-[12px] font-bold uppercase tracking-[0.25em]">Your information is safe. We respond within 24 hours on business days.</p>
              <div className="flex flex-wrap justify-center gap-x-12 gap-y-5">
                {["Free Design Mockup", "No Hidden Fees", "Sample Before Bulk", "Reply in 24 Hours"].map((item, i) => (
          ...
          {status === 'success' && <div className="p-8 bg-green-500/10 border border-green-500/20 rounded-3xl text-center"><p className="text-green-500 font-black uppercase text-xs tracking-[0.3em] animate-bounce">&#10003; Successfully Sent!</p></div>}
          {status === 'error' && <div className="p-8 bg-red-500/10 border border-red-500/20 rounded-3xl text-center"><p className="text-red-500 font-black uppercase text-xs tracking-[0.3em]">&#10005; Error sending message.</p></div>}
        </form>
      </div>
    </div>
  );
